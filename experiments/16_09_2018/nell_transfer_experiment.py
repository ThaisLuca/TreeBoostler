...
experiments = [
            ...
            #{'source':'nell_sports', 'target':'nell_finances', 'predicate':'teamalsoknownas'},
            {'source':'nell_finances', 'target':'nell_sports', 'predicate':'companyalsoknownas'},
...
while results['save']['n_runs'] < n_runs:
    ...
    results['save']['experiment'] = experiment
#    try:
    ...
    n_folds = len(tar_total_data[0]) if target not in ['nell_sports', 'nell_finances'] else 4
    results_save = []
    for i in range(n_folds):     
        ob_save = {}
        
        #[tar_train_pos, tar_test_pos] = datasets.get_kfold_small(i, tar_total_data[0])
        tar_train_pos = tar_total_data[0][0]
        ...
        # Group and shuffle
        if target in ['nell_sports', 'nell_finances']:
            tar_train_facts = tar_data[0][0]
            tar_test_facts = tar_data[0][0]
            pos_s = datasets.split_into_folds(tar_data[1][0], n_folds=4)
            neg_s = datasets.split_into_folds(tar_data[2][0], n_folds=4)
            [tar_train_pos, tar_test_pos] =  datasets.get_kfold_small(i, pos_s)
            [tar_train_neg, tar_test_neg] =  datasets.get_kfold_small(i, neg_s)
        else:
