...
while results['save']['experiment'] < len(experiments) and results['save']['run'] < n_runs:
    ...
    #for i in range(n_folds):
        ...
        for small_train in range(len(order)):
            ...
            [model, total_revision_time, inference_time, t_results, structured, pl_inference_time, pl_t_results] = theory_revision(background, boostsrl, target, s_train_pos, s_train_neg, s_train_facts, validation_pos, validation_neg, test_pos, test_neg, test_facts, transferred_structured, trees=10, max_revision_iterations=10, verbose=verbose)
