...
def get_tree_helper(path, nodes, leaves, variances):
    ...
    varc = variances[path]
    if left in nodes:
        children[0] = get_tree_helper(left, nodes, leaves, variances)
    if right in nodes:
        children[1] = get_tree_helper(right, nodes, leaves, variances)
    if left in leaves:
...
def get_tree(nodes, leaves, variances):
    return get_tree_helper('', nodes, leaves, variances)
...
def generalize_tree_helper(root):
    if isinstance(root, list):
        ...
        children= value[1]
        variances = value[0]
        true_child = generalize_tree_helper(children[0])
        false_child = generalize_tree_helper(children[1])
